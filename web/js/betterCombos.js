...
const CHECKPOINT_LOADER = "CheckpointLoader|jk-nodes";
const LORA_LOADER = "LoraLoader|jk-nodes";
...
app.registerExtension({
	...
	init() {
		$el("style", {
			textContent: `
				.litemenu-entry:hover .jk-nodes-combo-image {
					display: block;
				}
				.jk-nodes-combo-image {
					display: none;
					position: absolute;
					left: 0;
					top: 0;
		...
		const symbol = getOrSet(window, "__jk-nodes__", () => Symbol("__jk-nodes__"));
		...
		contextMenuHook["addItem"].push(function (el, menu, [name, value, options]) {
			if (el && isCustomItem(value) && value?.image && !value.submenu) {
				el.textContent += " *";
				$el("div.jk-nodes-combo-image", {
					parent: el,
					style: {
						backgroundImage: `url(/jk-nodes/view/${encodeRFC3986URIComponent(value.image)})`,
					},
				});
			}
		});
		...
		if (isCkpt || isLora) {
			...
			nodeType.prototype.onAdded = function () {
				...
				const get = async (route, suffix) => {
					...
					return await api.fetchApi(`/jk-nodes/${route}/${url}`);
				};
				...
				nodeType.prototype["jk-nodes.updateExamples"] = listExamples;
								...
								options: nodes.map((n) => ({
									...
									callback: async () => {
										...
										const { image } = await api.fetchApi(
											"/jk-nodes/save/" + encodeURIComponent(`${getType(n)}/${n.widgets[0].value.content}`),
											{
												method: "POST",
												body: JSON.stringify({
