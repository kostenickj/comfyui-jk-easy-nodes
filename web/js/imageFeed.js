...
$el("style", {
	...
	div > .jk-nodes-image-feed {
		position: static;
	}
	.jk-nodes-image-feed--top, .jk-nodes-image-feed--bottom {
		width: 100vw;
		min-height: 30px;
		max-height: calc(var(--max-size, 20) * 1vh);
	}
	.jk-nodes-image-feed--top {
		top: 0;
	}
	.jk-nodes-image-feed--bottom {
		bottom: 0;
		flex-direction: column-reverse;
		padding-top: 5px;
	}
	.jk-nodes-image-feed--left, .jk-nodes-image-feed--right {
		top: 0;
		height: 100vh;
		min-width: 200px;
		max-width: calc(var(--max-size, 10) * 1vw);
	}
	.comfyui-body-left .jk-nodes-image-feed--left, .comfyui-body-right .jk-nodes-image-feed--right {
		height: 100%;
	}
	.jk-nodes-image-feed--left {
		left: 0;
	}
	.jk-nodes-image-feed--right {
		right: 0;
	}

	.jk-nodes-image-feed--left .jk-nodes-image-feed-menu, .jk-nodes-image-feed--right .jk-nodes-image-feed-menu {
		flex-direction: column;
	}

	.jk-nodes-image-feed-menu {
		position: relative;
	...
	.jk-nodes-image-feed-btn.sizing-btn:checked {
		filter: invert();
	}
	.jk-nodes-image-feed-btn.clear-btn {
		padding: 5px 20px;
	}
	.jk-nodes-image-feed-btn.hide-btn {
		padding: 5px;
		aspect-ratio: 1 / 1;
	}
	.jk-nodes-image-feed-btn:hover {
		filter: brightness(1.2);
	}
	.jk-nodes-image-feed-btn:active {
		position:relative;
		top:1px;
	}
	
	.jk-nodes-image-feed-menu section {
		border-radius: 5px;
	...
	.jk-nodes-image-feed-menu section span {
		white-space: nowrap;
	}
	.jk-nodes-image-feed-menu section input {
		flex: 1 1 100%;
		background: rgba(0,0,0,0.6);
		border-radius: 5px;
		overflow: hidden;
	...
	.jk-nodes-image-feed--bottom .size-controls-flyout  {
		transform: scale(1,0);
		transform-origin: bottom;
		bottom: 0;
		left: 0;
	}
	.jk-nodes-image-feed--top .size-controls-flyout  {
		transform: scale(1,0);
		transform-origin: top;
		top: 0;
		left: 0;
	}
	.jk-nodes-image-feed--left .size-controls-flyout  {
		transform: scale(0, 1);
		transform-origin: left;
		top: 0;
		left: 0;
	}
	.jk-nodes-image-feed--right .size-controls-flyout  {
		transform: scale(0, 1);
	...
	.jk-nodes-image-feed-list:empty {
		display: none;
	}
	.jk-nodes-image-feed-list div {
		height: 100%;
		text-align: center;
	}
	.jk-nodes-image-feed-list::-webkit-scrollbar {
		background: var(--comfy-input-bg);
		border-radius: 5px;
	}
	.jk-nodes-image-feed-list::-webkit-scrollbar-thumb {
		background:var(--comfy-menu-bg);
		border: 5px solid transparent;
		border-radius: 8px;
		background-clip: content-box;
	}
	.jk-nodes-image-feed-list::-webkit-scrollbar-thumb:hover {
		background: var(--border-color);
		background-clip: content-box;
	}
	.jk-nodes-image-feed-list img {
		object-fit: var(--img-fit, contain);
		max-width: 100%;
		max-height: calc(var(--max-size) * 1vh);
		border-radius: 4px;
	}
	.jk-nodes-image-feed-list img:hover {
		filter: brightness(1.2);
	}`,
...
app.registerExtension({
	...
	async setup() {
		...
		const getVal = (n, d) => {
			const v = localStorage.getItem("jk-nodes.ImageFeed." + n);
			if (v && !isNaN(+v)) {
				return v;
			}
			return d;
		...
		const saveVal = (n, v) => {
			localStorage.setItem("jk-nodes.ImageFeed." + n, v);
		};

		const imageFeed = $el("div.jk-nodes-image-feed");
		const imageList = $el("div.jk-nodes-image-feed-list");
		...
		const feedLocation = app.ui.settings.addSetting({
			id: "jk-nodes.ImageFeed.Location",
						...
						$el(
							"select",
							{
								...
								oninput: (e) => {
									...
									imageFeed.className = `jk-nodes-image-feed jk-nodes-image-feed--${feedLocation.value}`;
			...
			onChange(value) {
				if (value === "hidden") {
					...
					imageFeed.className = `jk-nodes-image-feed jk-nodes-image-feed--${value}`;
		...
		const feedDirection = app.ui.settings.addSetting({
			id: "jk-nodes.ImageFeed.Direction",
		...
		const deduplicateFeed = app.ui.settings.addSetting({
			id: "jk-nodes.ImageFeed.Deduplication",
...
Recommended: "enabled (max performance)" uness images are erroneously deduplicated.`,
		...
		const maxImages = app.ui.settings.addSetting({
			id: "jk-nodes.ImageFeed.MaxImages",
		...
		const clearButton = $el("button.jk-nodes-image-feed-btn.clear-btn", {
			textContent: "Clear",
			onclick: () => {
		...
		const hideButton = $el("button.jk-nodes-image-feed-btn.hide-btn", {
			textContent: "❌",
			onclick: () => {
		...
		imageFeed.append(
			$el("div.jk-nodes-image-feed-menu", [
				...
				$el("div.jk-nodes-image-feed-btn-group", {}, [clearButton, hideButton]),
			]),
			imageList
		);
		showButton.onclick = () => {
