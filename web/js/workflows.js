...
.jk-nodes-workflow-arrow:after {
   content: "▼";
}
.jk-nodes-workflow-arrow:hover {
   filter: brightness(1.6);
   background-color: var(--comfy-menu-bg);
}
.jk-nodes-workflow-load .litemenu-entry:not(.has_submenu):before,
.jk-nodes-workflow-load ~ .litecontextmenu .litemenu-entry:not(.has_submenu):before {
	content: "🎛️";
	padding-right: 5px;
}
.jk-nodes-workflow-load .litemenu-entry.has_submenu:before,
.jk-nodes-workflow-load ~ .litecontextmenu .litemenu-entry.has_submenu:before {
	content: "📂";
	padding-right: 5px;
	position: relative;
	top: -1px;
}
.jk-nodes-workflow-popup ~ .litecontextmenu {
	transform: scale(1.3);
}
`;

async function getWorkflows() {
	const response = await api.fetchApi("/jk-nodes/workflows", { cache: "no-store" });
	return await response.json();
}

async function getWorkflow(name) {
	const response = await api.fetchApi(`/jk-nodes/workflows/${encodeURIComponent(name)}`, { cache: "no-store" });
	return await response.json();
}

async function saveWorkflow(name, workflow, overwrite) {
	try {
		const response = await api.fetchApi("/jk-nodes/workflows", {
			method: "POST",
			headers: {
...
class jk-nodesWorkflows {
	...
	constructor() {
		function addWorkflowMenu(type, getOptions) {
			return $el("div.jk-nodes-workflow-arrow", {
				...
				onclick: (e) => {
					...
					menu.root.classList.add("jk-nodes-workflow-popup");
					menu.root.classList.add(`jk-nodes-workflow-${type}`);
				},
			});
		}
...
app.registerExtension({
	...
	async setup() {
		workflows = new jk-nodesWorkflows();
