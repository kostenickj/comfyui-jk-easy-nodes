...
$el("style", {
	...
	.jk-image-feed--top, .jk-image-feed--bottom {
		width: 100vw;
		min-height: 30px;
	...
	.jk-image-feed--bottom {
		bottom: 0;
		flex-direction: column-reverse;
		padding-top: 5px;
	}
	.jk-image-feed--left, .jk-image-feed--right {
		top: 0;
		height: 100vh;
		min-width: 200px;
		max-width: calc(var(--max-size, 10) * 1vw);
	}
	.comfyui-body-left .jk-image-feed--left, .comfyui-body-right .jk-image-feed--right {
		height: 100%;
	}
	.jk-image-feed--left {
		left: 0;
	...
	.jk-image-feed--left .jk-image-feed-menu, .jk-image-feed--right .jk-image-feed-menu {
		flex-direction: column;
	}
	...
	.jk-image-feed-btn.sizing-btn:checked {
		filter: invert();
	}
	.jk-image-feed-btn.clear-btn {
		padding: 5px 20px;
	}
	.jk-image-feed-btn.hide-btn {
		padding: 5px;
		aspect-ratio: 1 / 1;
	}
	.jk-image-feed-btn:hover {
		filter: brightness(1.2);
	}
	.jk-image-feed-btn:active {
		position:relative;
		top:1px;
	}
	
	.jk-image-feed-menu section {
		border-radius: 5px;
	...
	.jk-image-feed-menu section span {
		white-space: nowrap;
	}
	.jk-image-feed-menu section input {
		flex: 1 1 100%;
		background: rgba(0,0,0,0.6);
		border-radius: 5px;
		overflow: hidden;
	...
	.jk-image-feed--bottom .size-controls-flyout  {
		transform: scale(1,0);
		transform-origin: bottom;
		bottom: 0;
		left: 0;
	}
	.jk-image-feed--top .size-controls-flyout  {
		transform: scale(1,0);
		transform-origin: top;
		top: 0;
		left: 0;
	}
	.jk-image-feed--left .size-controls-flyout  {
		transform: scale(0, 1);
		transform-origin: left;
		top: 0;
		left: 0;
	}
	.jk-image-feed--right .size-controls-flyout  {
		transform: scale(0, 1);
	...
	.jk-image-feed-list:empty {
		display: none;
	}
	.jk-image-feed-list div {
		height: 100%;
		text-align: center;
	}
	.jk-image-feed-list::-webkit-scrollbar {
		background: var(--comfy-input-bg);
		border-radius: 5px;
	}
	.jk-image-feed-list::-webkit-scrollbar-thumb {
		background:var(--comfy-menu-bg);
		border: 5px solid transparent;
		border-radius: 8px;
		background-clip: content-box;
	}
	.jk-image-feed-list::-webkit-scrollbar-thumb:hover {
		background: var(--border-color);
		background-clip: content-box;
	}
	.jk-image-feed-list img {
		object-fit: var(--img-fit, contain);
		max-width: 100%;
		max-height: calc(var(--max-size) * 1vh);
		border-radius: 4px;
	}
	.jk-image-feed-list img:hover {
		filter: brightness(1.2);
	}`,
...
app.registerExtension({
    ...
    async setup() {
        ...
        let showMenuButton;
        if (!((_a = app.menu) === null || _a === void 0 ? void 0 : _a.element.style.display) && ((_b = app.menu) === null || _b === void 0 ? void 0 : _b.settingsGroup)) {
        ...
        const getVal = (n, d) => {
            const v = localStorage.getItem("jk.ImageFeed." + n);
            if (v && !isNaN(+v)) {
                return v;
            }
            return d;
        ...
        const saveVal = (n, v) => {
            localStorage.setItem("jk.ImageFeed." + n, v);
        };
        ...
        const feedLocation = app.ui.settings.addSetting({
                    ...
                    $el("td", [
                        $el("select", {
                            ...
                            oninput: (e) => {
                                ...
                                imageFeed.className = `jk-image-feed jk-image-feed--${feedLocation.value}`;
                                updateMenuParent(feedLocation.value);
                                window.dispatchEvent(new Event("resize"));
                            },
                        }, ["left", "top", "right", "bottom", "hidden"].map((m) => $el("option", {
                            value: m,
                            textContent: m,
            ...
            onChange(value) {
                if (value === "hidden") {
                    ...
                    imageFeed.className = `jk-image-feed jk-image-feed--${value}`;
                    updateMenuParent(value);
                }
            },
        });
        const feedDirection = app.ui.settings.addSetting({
            id: "jk.ImageFeed.Direction",
            ...
            type: () => {
                return $el("tr", [
                    ...
                    $el("td", [
                        ...
                        }, ["newest first", "oldest first"].map((m) => $el("option", {
                            value: m,
                            textContent: m,
...
Recommended: "enabled (max performance)" uness images are erroneously deduplicated.`,
        ...
        const clearButton = $el("button.jk-image-feed-btn.clear-btn", {
            textContent: "Clear",
            onclick: () => {
        ...
        const hideButton = $el("button.jk-image-feed-btn.hide-btn", {
            textContent: "❌",
            onclick: () => {
        ...
        imageFeed.append($el("div.jk-image-feed-menu", [
            ...
            $el("div.jk-image-feed-btn-group", {}, [clearButton, hideButton]),
        ]), imageList);
        showButton.onclick = () => {
            window.onmessage = (e) => {
        ...
        api.addEventListener("executed", ({ detail }) => {
            var _a, _b, _c;
            if (visible && ((_a = detail === null || detail === void 0 ? void 0 : detail.output) === null || _a === void 0 ? void 0 : _a.images)) {
                if ((_c = (_b = detail.node) === null || _b === void 0 ? void 0 : _b.includes) === null || _c === void 0 ? void 0 : _c.call(_b, ":")) {
                    const n = app.graph.getNodeById(detail.node.split(":")[0]);
                    if (n === null || n === void 0 ? void 0 : n.getInnerNodes)
                        return;
                }
